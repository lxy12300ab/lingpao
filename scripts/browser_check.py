...
with sync_playwright() as p:
    ...
    page.goto('https://localhost:'+env.get('HTTPS_PORT','8443')+'/upload')
    expect(page.locator('#loginDialog')).to_be_visible()
    assert page.locator('#uploadDialog').evaluate('(el)=>el.tagName')=='MAIN'
    ...
    page.reload()
    expect(page.locator('#uploadDialog')).to_be_visible()
    page.goto('https://localhost:'+env.get('HTTPS_PORT','8443')+'/?action=upload')
    expect(page.locator('#uploadDialog')).to_be_visible()
    page.locator('#screenshotFile').set_input_files(str(root/'app/static/c11_hero.jpg'))
    page.evaluate('window.dispatchEvent(new PageTransitionEvent("pageshow", {persisted:true}))')
    expect(page.locator('#uploadPreview')).to_be_visible()
    assert page.locator('#screenshotFile').evaluate('(el)=>el.files.length')==1
    page.set_viewport_size({'width':390,'height':844})
    for scheme in ['light','dark']:
        page.emulate_media(color_scheme=scheme)
        expect(page.locator('html')).to_have_attribute('data-theme',scheme)
        assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
        page.screenshot(path=str(out/(engine+'-upload-page-'+scheme+'.png')),full_page=True)
    page.emulate_media(color_scheme='light')
    page.set_viewport_size({'width':1440,'height':1000})
    page.locator('[data-home]').click()
    page.goto('https://localhost:'+env.get('HTTPS_PORT','8443')+'/')
    expect(page.locator('#connection')).to_contain_text('已同步')
    expect(page.locator('#uploadDialog')).not_to_be_visible()
    checks.append('standalone upload page: login, reload, query alias, mobile themes, draft preserved on resume, home navigation')
