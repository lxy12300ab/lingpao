...
点击出行日历中的日期，可同时保存公里数与当天记录（最多 2000 字）。
没有里程时可以只写文字，不会把缺失里程当作 0；有文字的日期显示笔形标记。
文字独立存储，OCR 不会覆盖；清空文字并保存即可移除。多设备编辑冲突时需刷新后重试。
升级启动时自动创建文字记录表，无需重新导入旧数据。数据库备份包含文字记录；
原有 JSON 里程导出不包含文字，请使用数据库备份完整保存手记。
# 年度里程额度预警（2026.09.24.1）

首页“年度里程预警 → 仪表校准”设置周年起点、年度上限、当前周期起点仪表、
读数日期及仪表总里程。额度只是用户自设提醒，不判定官方质保资格。
日中读数仅显示该次校准额度，不叠加当天或后续里程，需更新为日终读数。
日终读数后累加后续每日记录；缺失已结束日期则提示剩余额度仅为上限，不给预测。
年度切换不删除历史数据，必须录入新周期起点读数，否则不显示确定额度。
配置与历次校准审计保存在 SQLite，随数据库备份。提醒仅在页面内展示，不发送手机推送。
